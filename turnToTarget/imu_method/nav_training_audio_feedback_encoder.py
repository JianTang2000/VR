...
# #######################这部分是超参数#########################
ip = str("192.168.0.105")  # IP 和 Unity 所在主机 IP要一致
...
angle_error = 5  # 误差在此范围内则认为已经对齐
reached_distance = 0.3  # 距离在次距离内则认为已经到达了目标
...
def play_by_angle(player_rotation_y, direction_lr, angle, remaining_distance):
    """
    ...
    if remaining_distance <= reached_distance:
        if abs(float(angle)) <= angle_error:
            speak_str_windows("arrived, target in front of you.")
            return
        else:
            speak_str_windows(f"arrived, target on your {direction_lr.strip()} side.")
            return
    else:
