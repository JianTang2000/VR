...
def get_img_depth():
    ...
    # 彩色图 720*1280*3 ,  彩色深度图 720*1280*3,  矩阵形式的深度信息图 720*1280(黑白)（也可以可视化）（乘以了scale所以是距离值）
...
def main():
    ...
    while True:
        ...
        _y_min_block = 490 - 10  # 矩形区域上方坐标（最上=0，最下=720）
        _y_max_block = 490 + 10  # 矩形区域下方坐标
        method_name(_depth_colormap, _depth_image_matrix, distance_min, rectangle_color, txt_color, _y_max_block, _y_min_block)
        _y_min_block = 540 - 10  # 矩形区域上方坐标（最上=0，最下=720）
        _y_max_block = 540 + 10  # 矩形区域下方坐标
        method_name(_depth_colormap, _depth_image_matrix, distance_min, rectangle_color, txt_color, _y_max_block, _y_min_block)
        _y_min_block = 590 - 10  # 矩形区域上方坐标（最上=0，最下=720）
        _y_max_block = 590 + 10  # 矩形区域下方坐标
        method_name(_depth_colormap, _depth_image_matrix, distance_min, rectangle_color, txt_color, _y_max_block, _y_min_block)
        _y_min_block = 640 - 10  # 矩形区域上方坐标（最上=0，最下=720）
        _y_max_block = 640 + 10  # 矩形区域下方坐标
        method_name(_depth_colormap, _depth_image_matrix, distance_min, rectangle_color, txt_color, _y_max_block, _y_min_block)
        _y_min_block = 690 - 10  # 矩形区域上方坐标（最上=0，最下=720）
        _y_max_block = 690 + 10  # 矩形区域下方坐标
        method_name(_depth_colormap, _depth_image_matrix, distance_min, rectangle_color, txt_color, _y_max_block, _y_min_block)
        # show
        if Vis:
            cv2.namedWindow('RGB')
...
def method_name(_depth_colormap, _depth_image_matrix, distance_min_, rectangle_color, txt_color, y_max_block_, y_min_block_):
    # distance_min_ = 0.01
    object_box1 = _depth_image_matrix[y_min_block_:y_max_block_, 2:181].astype(float)
    ...
    # put info on the img
    cv2.rectangle(_depth_colormap, (2, y_min_block_), (181, y_max_block_), rectangle_color, 1)
    cv2.rectangle(_depth_colormap, (185, y_min_block_), (364, y_max_block_), rectangle_color, 1)
    cv2.rectangle(_depth_colormap, (368, y_min_block_), (547, y_max_block_), rectangle_color, 1)
    cv2.rectangle(_depth_colormap, (551, y_min_block_), (730, y_max_block_), rectangle_color, 1)
    cv2.rectangle(_depth_colormap, (734, y_min_block_), (913, y_max_block_), rectangle_color, 1)
    cv2.rectangle(_depth_colormap, (917, y_min_block_), (1096, y_max_block_), rectangle_color, 1)
    cv2.rectangle(_depth_colormap, (1100, y_min_block_), (1278, y_max_block_), rectangle_color, 1)
    ...
    cv2.putText(_depth_colormap, txt, (185, int((y_min_block_ + y_max_block_) / 2)), 0, 0.5, txt_color, 1, 4)
    txt = str(round(dist3, 2)) + "," + str(no_depth_area3)
    cv2.putText(_depth_colormap, txt, (368, int((y_min_block_ + y_max_block_) / 2)), 0, 0.5, txt_color, 1, 4)
    txt = str(round(dist4, 2)) + "," + str(no_depth_area4)
    cv2.putText(_depth_colormap, txt, (551, int((y_min_block_ + y_max_block_) / 2)), 0, 0.5, txt_color, 1, 4)
    txt = str(round(dist5, 2)) + "," + str(no_depth_area5)
    cv2.putText(_depth_colormap, txt, (734, int((y_min_block_ + y_max_block_) / 2)), 0, 0.5, txt_color, 1, 4)
    txt = str(round(dist6, 2)) + "," + str(no_depth_area6)
    cv2.putText(_depth_colormap, txt, (917, int((y_min_block_ + y_max_block_) / 2)), 0, 0.5, txt_color, 1, 4)
    txt = str(round(dist7, 2)) + "," + str(no_depth_area7)
    cv2.putText(_depth_colormap, txt, (1100, int((y_min_block_ + y_max_block_) / 2)), 0, 0.5, txt_color, 1, 4)
