...
指定 jpg位置, 不会对这个路径下的文件进行任何增删改操作 （只会处理.jpg图片，且是三通道的照片，其他舍弃）
...
if __name__ == '__main__':
    ...
    # ####请手动在这里配置  img 存放位置
    img_dir = r'C:\Users\jiant\OneDrive\SJTU\7data\indoor-scene\1109ChairAndBottleTestData\img'
    # ####请手动在这里配置  xml 存放位置
    anno_dir = r'C:\Users\jiant\OneDrive\SJTU\7data\indoor-scene\1109ChairAndBottleTestData\xml'
    # ####请手动在这里配置  生成文件的目录 （会自动生成次级目录）
    save_path = r'C:\Users\jiant\OneDrive\SJTU\7data\indoor-scene\1109ChairAndBottleTestData\testDataBottleAndChair-toVal'
    ...
    img_dir_names = [name.replace('.jpg', '') for name in img_dir_file if name.endswith('.jpg')]
    ...
    good_set_have_img_xml = [i + ".jpg" for i in good_set_have_img_xml_no_end if True]
    ...
    pic_names = [name.replace('.jpg', '') for name in fileName1 if name.endswith('.jpg')]
    ...
    for name in pic_names_no_txt:
        file_d = os.path.join(img_val_GT, (name + '.jpg'))
        os.remove(file_d)
    ...
    pic_names_no_txt = [name for name in pic_names if name not in txt_names]
    for name in pic_names_no_txt:
        file_d = os.path.join(img_save_dir, (name + '.jpg'))
        os.remove(file_d)
